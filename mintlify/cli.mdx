...
| `--evaluator-result` | submit one or more harness result envelopes (an object or a JSON array) for outstanding work requests, from a file or - for stdin (requires --resume) |
...
| `--state` | filter runs: all, reportable, incomplete, awaiting (default: `all`) |
