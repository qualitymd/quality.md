...
3. **Format, not goodness.** It decides whether the file _is a valid
   QUALITY.md file_, never whether it is a _good_ quality model. Anything requiring
...
- **signal severity through its verb** — error rules _flag_ or _reject_, warning
  rules _warn_, info rules _report_ or _summarize_ — so the catalog reads
  consistently; and
...
the constraint it enforces; the _description_ column follows
...
| -------------------------- | -------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------ | ------- | ---------------------------------------------------------------------------------- |
| `invalid-frontmatter`      | _YAML Frontmatter_ — a file MUST begin with a valid YAML frontmatter block                         | Rejects frontmatter that is absent, not valid YAML, or not the model's shape.                    | No      | Parse and shape failures do not have one safe structural repair.                   |
| `missing-rating-scale`     | _Model_ — `ratingScale` is required                                                                | Flags a model that declares no `ratingScale`.                                                    | No      | The scale defines model semantics; the format has no single required scale.        |
| `missing-title`            | _Model / Area / Factor / Requirement / Rating scale_ — each displayable element requires a `title` | Flags a model, area, factor, requirement, or rating level with no non-empty scalar `title`.      | No      | A title names the element for readers and cannot be inferred mechanically.         |
| `too-few-levels`           | _Rating scale_ — at least two rating levels MUST be supplied                                       | Flags a `ratingScale` with fewer than two rating levels.                                         | No      | Adding levels requires choosing scale vocabulary and criteria.                     |
| `missing-level-name`       | _Rating scale_ — each level MUST declare a `level` name                                            | Flags a rating level that declares no `level` name.                                              | No      | A generated name would define rating vocabulary for the author.                    |
| `invalid-rating-level-id`  | _Names and model references_ — rating level IDs MUST match the strict name grammar                 | Flags a rating level whose `level` does not match the strict name grammar.                       | No      | Repair requires choosing the intended rating level ID.                             |
| `duplicate-level`          | _Rating scale_ — a `level` name MUST be unique within the scale                                    | Flags two rating levels that share a `level` name.                                               | No      | Repair requires choosing which level to rename, merge, or remove.                  |
| `invalid-config`           | _qualitymd tooling_ — root `config` MUST be a safe model-relative scalar path                      | Flags a root `config` value that is empty, non-scalar, absolute, or escapes the repository.      | No      | Repair requires choosing the intended workspace config file path.                  |
| `missing-criterion`        | _Rating scale_ — each level MUST declare a `criterion`                                             | Flags a rating level that declares no `criterion`.                                               | No      | Criterion text defines rating semantics and cannot be inferred mechanically.       |
| `empty-model`              | _Model_ — an entry on `factors`, `requirements`, or `areas` MUST be supplied                       | Flags a model root that supplies no entry under `factors`, `requirements`, or `areas`.           | No      | Repair requires choosing what kind of model content to add.                        |
| `misplaced-root-key`       | _Area_ — an area MUST NOT declare `ratingScale`                                                    | Flags an area that declares `ratingScale`.                                                       | No      | Repair requires deciding whether to remove, move, or reinterpret authored content. |
| `invalid-area-name`        | _Names and model references_ — area names MUST match the strict name grammar                       | Flags an area name that does not match the strict name grammar.                                  | No      | Repair requires choosing the intended area name.                                   |
| `reserved-area-name`       | _Names and model references_ — area names MUST NOT use the reserved `root` token                   | Flags an area name that uses the reserved root area reference token.                             | No      | Repair requires choosing the intended area name.                                   |
| `invalid-factor-name`      | _Names and model references_ — factor names MUST match the strict name grammar                     | Flags a factor name that does not match the strict name grammar.                                 | No      | Repair requires choosing the intended factor name.                                 |
| `invalid-requirement-name` | _Names and model references_ — requirement names MUST match the strict name grammar                | Flags a requirement name that does not match the strict name grammar.                            | No      | Repair requires choosing the intended requirement name.                            |
| `duplicate-requirement`    | _Requirement_ — requirement names MUST be unique within the declaring area                         | Flags a requirement name that appears more than once within one declaring area.                  | No      | Repair requires choosing which requirement to rename, merge, or remove.            |
| `invalid-assessment`       | _Requirement_ — a requirement MUST declare exactly one `assessment` as a non-empty scalar          | Flags a requirement whose `assessment` is missing, empty, or a list rather than a single scalar. | No      | Repair requires choosing the requirement's assessment text.                        |
| `unknown-factor`           | _Requirement_ — each factor reference MUST resolve to a factor declared on the requirement's area  | Flags a requirement whose `factors` entry references no factor on its declaring area.            | No      | Repair requires choosing a factor on the same area or adding one there.            |
| `missing-factor-reference` | _Requirement_ — every requirement MUST be connected to at least one factor                         | Flags a direct area-level requirement with no non-empty scalar factor reference.                 | No      | Repair requires choosing the intended factor or moving the requirement under one.  |
| `unknown-rating-key`       | _Requirement_ — each `ratings` override key MUST name a level of the rating scale                  | Flags a `ratings` override key that names no level in the model's `ratingScale`.                 | No      | Repair requires choosing the intended rating level or changing the scale.          |
