...
| `missing-title`            | *Model / Area / Factor / Requirement / Rating Scale* — each displayable element requires a `title` | Flags a Model, Area, Factor, Requirement, or Rating Level with no non-empty scalar `title`.      | No      | A title names the element for readers and cannot be inferred mechanically.         |
...
| `invalid-rating-level-id`  | *Names and Model References* — Rating Level IDs MUST match the strict name grammar                 | Flags a rating level whose `level` does not match the strict name grammar.                       | No      | Repair requires choosing the intended Rating Level ID.                             |
...
| `invalid-config`           | *qualitymd tooling* — root `config` MUST be a safe model-relative scalar path                      | Flags a root `config` value that is empty, non-scalar, absolute, or escapes the repository.      | No      | Repair requires choosing the intended workspace config file path.                  |
...
| `invalid-area-name`        | *Names and Model References* — Area names MUST match the strict name grammar                       | Flags an Area name that does not match the strict name grammar.                                  | No      | Repair requires choosing the intended Area name.                                   |
| `reserved-area-name`       | *Names and Model References* — Area names MUST NOT use the reserved `root` token                   | Flags an Area name that uses the reserved root Area reference token.                             | No      | Repair requires choosing the intended Area name.                                   |
| `invalid-factor-name`      | *Names and Model References* — Factor names MUST match the strict name grammar                     | Flags a Factor name that does not match the strict name grammar.                                 | No      | Repair requires choosing the intended Factor name.                                 |
| `invalid-requirement-name` | *Names and Model References* — Requirement names MUST match the strict name grammar                | Flags a Requirement name that does not match the strict name grammar.                            | No      | Repair requires choosing the intended Requirement name.                            |
| `duplicate-requirement`    | *Requirement* — Requirement names MUST be unique within the declaring Area                         | Flags a Requirement name that appears more than once within one declaring Area.                  | No      | Repair requires choosing which Requirement to rename, merge, or remove.            |
...
| `unknown-factor`           | *Requirement* — each factor reference MUST resolve to a factor declared on the Requirement's Area  | Flags a requirement whose `factors` entry references no factor on its declaring Area.            | No      | Repair requires choosing a factor on the same Area or adding one there.            |
...
| `duplicate-factor-name`      | *Factor* — an Area SHOULD avoid reusing one Factor name in its Factor tree | Warns when the same factor name appears more than once inside one Area's recursive Factor tree.                                    | No      | Repair requires choosing whether to rename, merge, or keep the distinct Factors.              |
