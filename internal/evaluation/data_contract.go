	...
	for _, contract := range []dataKindContract{
		...
		},
		{
			...
			Object: topContract(DataKindRecommendation,
				...
				field("whyItMatters", dataString, true),
				field("recommendedNextMove", dataString, true),
				field("expectedBenefit", dataString, true),
				field("howToKnowItWorked", dataString, true),
		...
		},
		{
			...
			Example: recommendationRankingExample,
		},
		{
			...
			Object: topContract(DataKindEvaluationOutput,
				...
				field("scopedAreaAnalysisRef", dataObject, true, routineRefContract()),
...
func runRequestedScopeContract() dataObjectContract {
	return object(
		field("areaId", dataAreaID, false),
		field("factorFilter", dataArray, false, arrayOf(dataFactorID)),
	)
}

func runPlannedScopeContract() dataObjectContract {
	return object(
		field("areaId", dataAreaID, true),
		field("factorFilter", dataArray, true, arrayOf(dataFactorID)),
	)
}
...
func reportRefContract() dataObjectContract {
	return object(
		...
		field("factorId", dataFactorID, false),
		field("requirementId", dataRequirementID, false),
		field("recommendationId", dataString, false),
...
func validatePayloadSemantics(kind DataKind, payload map[string]any) error {
	...
	case DataKindRecommendation:
		return validateRecommendationResultSemantics(payload)
	default:
		return nil
	}
}
...
func repeatedModelReferenceKey(key string) (string, bool) {
	...
	case "areaIds", "childAreaIds":
		return "areaId", true
	case "factorIds", "rootFactorIds", "factors", "factorFilter":
