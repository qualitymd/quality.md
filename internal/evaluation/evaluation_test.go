...
func TestSetDataAndBuildEvaluationReport(t *testing.T) {
	...
	for _, payload := range completeRootEvaluationPayloads() {
...
func TestSetDataRejectsUnknownFieldsAndUnresolvedModelReferences(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			if _, err := SetData(runPath, []byte(tc.raw), DataSetOptions{DryRun: true}); err == nil || !strings.Contains(err.Error(), tc.want) {
