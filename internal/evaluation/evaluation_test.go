...
func TestAddRecordStatusAndBuildReport(t *testing.T) {
  ...
  "assessmentResultRecords": [
	...
	for _, want := range []string{"## Verdict", "## Scope", "## Selected Findings and Limitations", "## Evidence Basis", "## Next Action", "## Area Breakdown", "- **Evaluation verdict:** 🟡 Minimum", "| Test model | `area:root` | 🟡 Minimum | 🟡 Minimum | (no factor ratings) |"} {
	...
	for _, want := range []string{"# Quality Evaluation Summary", "| Run |", "| Scope | Full evaluation |", "| Evaluation verdict | 🟡 Minimum |", "[report.md](report.md)", "[report.json](report.json)", "## Verdict", "## Area Breakdown", "| Area | Path | Area Rating | Area + Sub-Areas Rating | Factors |", "| Test model | `area:root` | 🟡 Minimum | 🟡 Minimum | (no factor ratings) |", "## Selected Findings", "**Medium**", "Only a smoke test exists.", "## Recommended Actions", "| Recommendation ID | Priority | Recommendation | Done criterion |", "`001-fix-the-test-gap`", "## Scope & Limitations"} {
...
`)
  ...
  "assessmentResultRecords": []
	...
	for _, want := range []string{
		...
		"| Root Quality Model | `area:root` | (area group) | Good | Operational reliability: Good |",
		"| API Service | `area:api-service` | Good | Good | Automation compatibility: Good; Operational reliability: Good |",
		"### API Service",
		"- **Path:** api-service",
		"- **Area rating:** Good",
