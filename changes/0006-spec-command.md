...
The [`qualitymd spec`](../specs/cli/spec.md) sub-spec is still a placeholder
stub, and the command is unimplemented — invoking it fails with "unknown
command". [Change 0004](archive/0004-specify-agent-accessibility.md) settled the
...
baseline." This is that change: it settles the durable sub-spec and lands the
command. The detail lives in its children:
...
- [Design doc](0006-spec-command/design.md) - how the bundled specification is
  embedded and emitted. Added when the change advances to **Design**.
...
- [ ] [`specs/cli/spec.md`](../specs/cli/spec.md) — replace the placeholder stub
...
- [ ] [`README.md`](../README.md) — move `spec` from planned to built in the
      command status.
...
`Draft`. See the [status lifecycle](index.md#status-lifecycle). Functional spec
written; design and implementation follow.
