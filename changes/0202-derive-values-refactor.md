...
description: Bring src/ into conformance with the derive-values style rule and decompose evaluation-execute into pure domain functions, preserving public CLI behavior.
status: Design
...
Status note (two clocks): case is **Design**; code is untouched. The style-guide
section this case enforces already landed independently in
[`docs/guides/effect-typescript-style.md`](../docs/guides/effect-typescript-style.md)
("Derive values; do not accumulate them"), so that durable doc is ahead of the
code clock.
...
This case brings `src/` into conformance with the guide section, decomposes the
flagship file into pure, unit-tested domain functions, and replaces the five
enumeration copies with one shared, spec-backed implementation — all without
changing public CLI behavior beyond the one decided enumeration edge case.
...
- **Durable docs:** `docs/guides/effect-typescript-style.md` — already updated
  with the normative section; no further planned change.
...
- **Domain code:** new frame/request/summary module(s) under
  `src/domain/evaluation/`; `graph.ts` (ready-unit selection); `protocol.ts`
  (optional fields instead of sentinels); `run.ts` (run-folder name
  ...
  `document.ts`.
- **Application code:** `evaluation-execute.ts` (flagship decomposition);
...
- [Functional spec](0202-derive-values-refactor/spec.md) — behavior
  preservation, derivation conformance, decomposition, scheduling, and
  enumeration requirements.
- [Design doc](0202-derive-values-refactor/design.md) — module map, derivation
  idioms, hashing boundary, unified enumeration, and the exclusion decision.
