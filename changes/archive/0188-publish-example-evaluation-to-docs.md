...
`Done`. Implemented and archived. The nav link is in place, the generator and
generated pages are removed, and the `mise run check` gate no longer references
`report-docs`. See the [status lifecycle](../index.md#status-lifecycle).
