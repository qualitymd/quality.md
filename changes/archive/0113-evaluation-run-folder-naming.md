...
- [Functional spec](0113-evaluation-run-folder-naming/spec.md) - what the change
  must do.
- [Design doc](0113-evaluation-run-folder-naming/design.md) - how the change is
  built.
...
stays the identity and `model-snapshot.md` stays the structural source of truth;
the slug is a human mnemonic that now carries maximal scope context.
...
- [x] None - `SPECIFICATION.md` does not govern Evaluation v2 run-folder names.
      (Deliberate.)
...
- [x] `skills/quality/SKILL.md` - update the `run: 0003-quality-eval` handle
      example to `run: 0003-full-eval`, and the narrowing note to name the
      full-path slug convention.
- [x] `skills/quality/workflows/evaluate.md` - in the create-run step, construct
      `--narrowing` as the scope's full structural path; refresh any run-name
      example.
- [x] `skills/quality/resources/cli-quick-reference.md` - reconcile any
      run-name example and the `--narrowing` description.
- [x] `CHANGELOG.md` - add an Unreleased entry recording the run-folder name
      change (mirroring the prior naming-change note).
- [x] `evaluation-v2-sketch.md` - reconcile any run-folder examples that show the
      old tag.
