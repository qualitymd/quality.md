...
const resolveRun = (input: RunInput) =>
  Effect.gen(function* () {
    ...
    if (!(yield* fs.exists(workspace.evaluations.abs))) throw new Error("no evaluation runs found")
    const candidates: Array<{ readonly number: number; readonly name: string }> = []
    for (const name of yield* fs.readDirectory(workspace.evaluations.abs)) {
      const directory = paths.join(workspace.evaluations.abs, name)
      if ((yield* fs.stat(directory)).type !== "Directory") continue
      let number = Number(name.slice(0, 4))
      const artifact = paths.join(directory, "evaluation.json")
      if (yield* fs.exists(artifact)) {
        try {
          const value = JSON.parse(yield* fs.readFileString(artifact)) as {
            readonly manifest?: { readonly run?: { readonly number?: number } }
          }
          number = value.manifest?.run?.number ?? number
        } catch {
          // Keep the folder number for malformed runs; the command reports the real error later.
        }
      }
      if (Number.isFinite(number)) candidates.push({ number, name })
    }
    candidates.sort(
    ...
    return {
      absolute: paths.join(workspace.evaluations.abs, latest.name),
      display: `${workspace.evaluations.rel}/${latest.name}`,
    } satisfies ResolvedRun
  })
    ...
    const visit = (current: string): Effect.Effect<void, unknown> =>
      Effect.gen(function* () {
        for (const name of yield* fs.readDirectory(current)) {
          ...
          else if (info.type === "File" && name.endsWith(".json")) output.push(path)
        }
      })
...
export const validateCrossPayloads = (payloads: ReadonlyArray<Record<string, unknown>>) => {
  ...
  for (const payload of payloads) {
    ...
    const path = dataPathForPayload(kind, payload)
    if (paths.has(path)) throw new Error(`effective payloads duplicate ${path}`)
    paths.set(path, payload)
  }
  ...
  for (const [requirementId, assessment] of assessments) {
    for (const finding of objects(assessment.findings))
      findingKeys.add(`${String(requirementId)}#${String(finding.id)}`)
  }
  ...
  const validateRanks = (entries: ReadonlyArray<Record<string, unknown>>, name: string) => {
    ...
    for (const [index, entry] of entries.entries()) {
      ...
      if (seen.has(Number(rank))) throw new Error(`${name}[${index}].rank duplicates ${rank}`)
  ...
  for (const payload of payloads) {
    ...
    if (payload.kind === "FindingRankingResult") {
      ...
      for (const [index, entry] of entries.entries()) {
        const key = findingKey(entry.findingRef)
        if (!findingKeys.has(key))
          throw new Error(
            `orderedFindings[${index}].findingRef does not resolve to an in-scope finding`,
          )
        if (seen.has(key)) throw new Error(`orderedFindings[${index}].findingRef duplicates ${key}`)
        seen.add(key)
      }
      for (const key of findingKeys)
        if (!seen.has(key)) throw new Error(`orderedFindings missing ${key}`)
    ...
    if (
      ...
      for (const [index, value] of entries.entries()) {
        const recommendationRef = String(value.recommendationRef ?? "")
        ...
        if (ranked.has(recommendationRef))
          throw new Error(
            `orderedRecommendations[${index}].recommendationRef duplicates ${recommendationRef}`,
          )
        ranked.add(recommendationRef)
      }
      for (const id of recommendations)
        if (!ranked.has(String(id))) throw new Error(`orderedRecommendations missing ${String(id)}`)
      validateRanks(entries, "orderedRecommendations")
      const covered = new Set<string>()
      for (const [index, coverage] of objects(payload.findingCoverage).entries()) {
        const key = findingKey(coverage.findingRef)
        if (!findingKeys.has(key))
          throw new Error(
      ...
      for (const key of findingKeys)
        if (!covered.has(key)) throw new Error(`findingCoverage missing ${key}`)
    }
  }
}
      ...
      try {
        for (const [index, value] of values.entries()) {
          if (value === null || Array.isArray(value) || typeof value !== "object")
            throw new Error(`payload[${index}]: payload must be a JSON object`)
          ...
          candidates.push({ index, kind, path: dataPathForPayload(kind, payload), payload })
        }
        ...
        if (yield* fs.exists(dataRoot)) {
          for (const path of yield* recursiveJson(dataRoot)) {
            try {
              existing.push(JSON.parse(yield* fs.readFileString(path)) as Record<string, unknown>)
            } catch {
              // The direct validation above still reports candidate failures; verify owns corrupt files.
            }
          }
        }
...
export const evaluationDataListCommand = (
  ...
  mapFailure(
    Effect.gen(function* () {
      ...
      const artifacts: Array<{ readonly kind: DataKind; readonly path: string }> = []
      const dataRoot = paths.join(run.absolute, "data")
      if (yield* fs.exists(dataRoot)) {
        for (const path of yield* recursiveJson(dataRoot)) {
          try {
            const payload = JSON.parse(yield* fs.readFileString(path)) as Record<string, unknown>
            const kind = resolveDataKind(String(payload.kind))
            if (filter === undefined || filter === kind)
              artifacts.push({
                kind,
                path: paths.relative(run.absolute, path).replaceAll("\\", "/"),
              })
          } catch {
            // List only validly identifiable payloads; verify reports malformed files.
          }
        }
      }
      ...
      if (yield* fs.exists(dataRoot)) {
        for (const path of yield* recursiveJson(dataRoot)) {
          ...
          try {
            const payload = JSON.parse(yield* fs.readFileString(path)) as Record<string, unknown>
            checked += 1
            validateDataPayload(payload, model, true)
